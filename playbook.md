...
- **Times are 24-hour HH:MM** — Bank types them this way (01:00, 17:00, 23:00; never am/pm). The server stores the time verbatim, sorts the day's table in eating order with a night-owl rollover (00:00–04:59 sit at the END of the day, after 23:00), and resolves "today" in Bangkok time. **Night-owl day rule:** a meal eaten 00:00–~05:00 that continues a night out belongs to the day that night STARTED — log it on that date (pass `date`=yesterday) instead of the default today. Only a genuinely ambiguous case needs a one-line confirm.
