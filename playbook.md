...
- **Auto-push (never ask permission to save):** the moment a meal is settled, **save it automatically** — call `foodlog_upsert(date, meals=<full day list>)` right away and then show the updated table. Do NOT ask "do you want me to log this / save to Notion?" — saving is the default, always. — pass `meals` as a native array, each item `["HH:MM","dish",kcal,p,c,f]` (never a quoted/JSON string). Build the list from `foodlog_read`'s `meals` (so meals logged in another chat are preserved) plus the new one. The server rewrites the table AND recomputes kcal/p/c/f to match — do not pass kcal/p/c/f yourself when passing meals.
